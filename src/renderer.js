...
export class renderer_t
{
  constructor()
  {
    ...
    this.floor_tex = texture_load_url("assets/floor.png");
    this.ceil_tex = texture_load_url("assets/ceil.png");
  ...
  load_map_play(map)
  {
    ...
    map.listen("load", () => {
      ...
      for (const door_id in map.doors) {
        const [pos, side] = map.from_door_id(door_id);
        const door = map.doors[door_id];
        
        if (side)
          meshes_vertices[door.id - 1].push(...build_wall(pos, vec3_t.forward, vec3_t.up));
        else
          meshes_vertices[door.id - 1].push(...build_wall(pos, vec3_t.right, vec3_t.up));
      }
  ...
  load_map_edit(map)
  {
    ...
    this.spawn_tex = texture_load_url("assets/spawn.png");
  ...
  draw_spawn(camera)
  {
    ...
    const spawn_pos = new vec3_t(26.0, 0.0, 25.0);
    
    const translation_matrix = new mat4_t().init_translation(spawn_pos);
    ...
    gl.bindTexture(gl.TEXTURE_2D, this.spawn_tex);
    gl.drawArrays(gl.TRIANGLES, this.wall_mesh.offset, this.wall_mesh.count);
  }
  
  draw_wall(camera, game, tool)
  {
    if (tool.wall_id == 0)
      return;
    
    const pos = game.front_pos.copy();
    ...
    const y_rot = game.front_side ? Math.PI / 2.0 : 0;
  ...
  draw_room(camera, map)
  {
    ...
    for (const door_id in map.doors) {
      ...
      const y_rot = side ? Math.PI / 2.0 : 0;
      ...
      this.shader.set_mvp(mvp);
      this.shader.set_opacity(0.8);
      
      gl.bindTexture(gl.TEXTURE_2D, this.wall_tex[door.id - 1]);
      gl.drawArrays(gl.TRIANGLES, this.wall_mesh.offset, this.wall_mesh.count);
    }
  }
}
